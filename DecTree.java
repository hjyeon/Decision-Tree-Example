...
import java.util.*;
/**
 * This class is an extension of the class with the same name I wrote for CS540. 
...
public class DecTree {
	...
	/**
	 * Constructor to use if the user only wants few features to build a tree
	 * Takes data as a file 
	 * @param filePath
	 * @param features
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	public DecTree (String filePath, ArrayList<Integer> features) throws FileNotFoundException, IOException {
		this.features = features;
		this.numAttr = features.size();
		this.features.add(7); // for labels 1(survived),0(deceased)
		//this.trainData = DataParser.parseTrainingRecords(filePath);
		this.trainData = this.selectData(DataParser.parseTrainingRecords(filePath));
		//System.out.println(trainData.get(0));
		if (this.numAttr == 1) this.root = buildTreeWithOne(trainData);
		else this.root = buildTree(trainData);
		this.features.remove(this.features.size()-1);
	}
	
	/**
	 * Constructor to use if the user only wants few features to build a tree
	 * Takes a list of list as a data. 
	 * @param trainData
	 * @param features
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	public DecTree (List<ArrayList<Integer>> trainData, ArrayList<Integer> features) throws FileNotFoundException, IOException {
		this.features = features;
		this.numAttr = features.size();
		this.features.add(7); // for labels 1(survived),0(deceased)
		//this.trainData = DataParser.parseTrainingRecords(filePath);
		
		this.trainData = this.selectData(trainData);
		//System.out.println(this.trainData.get(0));
		if (this.numAttr == 1) this.root = buildTreeWithOne(this.trainData);
		else this.root = buildTree(this.trainData);
		this.features.remove(this.features.size()-1);
		//System.out.println(this.features);
	}
	/**
	 * Constructor for using all available features and takes data as a list of list 
	 * @param trainData
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	public DecTree(List<ArrayList<Integer>> trainData) throws FileNotFoundException, IOException {
	 ...
	 * @throws IOException
	 */
	public DecTree(String filePath) throws FileNotFoundException, IOException {
		this.features = new ArrayList<Integer>(); 
		for (int i = 1; i< 7 ; i ++) {
			features.add(i);
		}
	...
	public void processTestData(String filePath) throws FileNotFoundException, IOException {
		...
		this.features.add(7);
		this.testData = this.selectData(this.testData);
		this.features.remove(this.features.size()-1);
	}
	...
	private void printTreeHelp(DecTreeNode node, int numSpaces) {
		...
		//	System.out.print("else ");
			String message = "";
			if (node.classLabel==2) message = "survived"; 
			else message = "deceased";
	 ...
	 * The modification is not difficult - see DecForest.java if interested in. 
	...
	public ArrayList<Integer> crossValidation(int fold, String originalFilePath) throws FileNotFoundException, IOException {
		...
		this.testData = DataParser.parseTestRecords(originalFilePath);
		for (int i = 0 ; i < fold ; i ++) {
			...
			if (i == (fold - 1)) {
				...
				trainingSubset = this.trainData.subList(sizeOfSet*(i+1),this.trainData.size());
				testSubset = this.testData.subList(sizeOfSet*i, sizeOfSet*(i+1));
			}

			this.root=this.buildTree(trainingSubset);
	...
	*/
	...
	/**
	 * This prints how many people in the training set have died. 
	 * @return number of people survived
	 */
	public int numDeathCases() {
		int count = 0 ;

		for (int i = 0 ; i < this.trainData.size() ; i ++ ) {
			if (this.trainData.get(i).get(this.numAttr) == 4) count++;
		}
		System.out.println("Total Data Points:" + this.trainData.size()+" Died:" +count);
		return count;
	}
	...
	public void printCVAccuracy(int fold, String originalFilePath) throws FileNotFoundException, IOException {
		...
		for (int i = 0 ; i < dataSize ; i++) {
			//TODO: remove this
			//System.out.print("?"+this.trainData.get(i).get(0));
			if (this.trainData.get(i).get(this.numAttr) == result.get(i)) count++;
		}
		System.out.println(count + " correct out of " + dataSize);
